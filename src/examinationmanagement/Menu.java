...
public class Menu {
    ...
    public int getSelectionMenu(int level) {
        showMenu(level);
        int option = Utils.inputInteger("Please enter your choice", 0, 10);
    ...
    private void showExitMenu() {
        ...
        System.out.println(MenuItem.MenuShow.toString());
        System.out.println(MenuItem.MenuExit.toString());
    }
            ...
            for (MenuItem item : MenuItem.values()) {
                if (level == item.getId() && item.getParent() == 0) {
                    ...
                    if (item.isIsParent()) {
                        ...
                        System.out.println(getIndent(item.getLevel()) + item.toString());
                    }
                }
            }
        }
